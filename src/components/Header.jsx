  ...
  return (
    <>
      ...
      <div className="fixed top-4 left-0 w-full z-40 px-4 pointer-events-none flex justify-center">
        <nav className="relative glass-panel rounded-full w-full max-w-5xl px-4 md:px-6 py-2 md:py-3 flex justify-between items-center pointer-events-auto shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
          ...
          <div className="hidden lg:flex gap-8 items-center h-[48px]">
            {navLinks.map(link => (
              <a key={link.id} href={link.href} className="text-sm font-semibold text-gray-300 hover:text-amber-400 transition-colors flex items-center h-full px-2">
                {link.name}
              </a>
            ))}
          </div>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm pointer-events-auto" onClick={() => setMobileMenuOpen(false)}>
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25 }} className="w-4/5 max-w-sm h-full glass-panel border-l border-white/10 p-6 sm:p-8 flex flex-col" onClick={e => e.stopPropagation()}>
